...
Run `composer build` (or `./scoper.sh -p PhpScoperExampleVendor`) to build the `vendor_prefixed` directory. This will:
...
- Replace hash from auto-generated `ComposerAutoloaderInit`, otherwise it will collide with the `ComposerAutoloaderInit` in the `vendor` directory (when running tests etc.)
- Create new hash for static files. PHP-scoper has prefixed some functions, these files would not load if the same static files have been loaded in another `autoload.php`.
...
1. Copy `scoper.inc.php`, replace `'exclude-namespaces' => ['Otomaties\\PhpScoperExample']` at the bottom of the file with the namespace of your plugin.
2. Copy `scoper.sh`
